...
namespace ITATS\PraktikumTeknikSipil\Http\Controllers\Dashboard\Praktikan;
...
class ProfilePraktikanController {
    ...
    public function Index() {
        ...
        View::render('dashboard.praktikan.profile', [
    ...
    public function UpdatePhoto($uid) {
        if (Helper::is_post() && Helper::is_csrf()) {
            if (!isset($_FILES['avatar']) || $_FILES['avatar']['error'] === UPLOAD_ERR_INI_SIZE) {
                return Helper::redirect('/dashboard/praktikan/profile', 'error', 'Ukuran file terlalu besar. Maksimal 2MB.');
            }
    
            $newPicture = $_FILES['avatar'];
            if (!$newPicture || $newPicture['error'] !== UPLOAD_ERR_OK) {
                return Helper::redirect('/dashboard/praktikan/profile', 'error', 'Tidak ada file yang diunggah atau file rusak.');
            }
    
            $fileName = $this->processImage($newPicture, $uid);
            if ($fileName instanceof Exception) {
                return Helper::redirect('/dashboard/praktikan/profile', 'error', 'Update photo gagal: ' . $fileName->getMessage());
            }
    
            try {
                $this->ProfileModel->UpdatePhoto($uid, $fileName);
                $_SESSION['user']['profile_picture'] = $fileName;
                return Helper::redirect('/dashboard/praktikan/profile', 'success', 'Foto profil berhasil diperbarui', 0, [
                    'profile_picture_url' => Helper::url('/file.php?file=user-pictures/' . $fileName)
                ]);
            } catch (Exception $e) {
                return Helper::redirect('/dashboard/praktikan/profile', 'error', 'Gagal update DB: ' . $e->getMessage());
            }
        }
    }
