...
//     public static function group(string $prefix, array $groupMiddlewares, callable $callback) {
//         // Simpan jumlah routes sebelum menjalankan callback
//         $previousRoutesCount = count(self::$routes);

//         // Jalankan callback untuk menambahkan sub-routes
//         call_user_func($callback);

//         // Ambil routes baru yang ditambahkan di callback
//         $newRoutes = array_slice(self::$routes, $previousRoutesCount);

//         // Hapus routes baru dari array utama sementara
//         self::$routes = array_slice(self::$routes, 0, $previousRoutesCount);

//         // Modifikasi setiap new route: tambah prefix dan merge middlewares
//         foreach ($newRoutes as $route) {
//             // Gabung prefix ke path regex
//             // Hilangkan leading/trailing slash berlebih dan sesuaikan regex
//             $originalPath = $route['path'];
//             $cleanPrefix = rtrim($prefix, '/');
//             $cleanSubPath = preg_replace('/^#\^\/?/', '', $originalPath); // Hilangkan #^/ awal
//             $cleanSubPath = preg_replace('/\$\#$i/', '', $cleanSubPath); // Hilangkan $#i akhir sementara
//             $newCompiledPath = "#^" . $cleanPrefix . (empty($cleanSubPath) ? '' : '/') . $cleanSubPath . "$#i";

//             // Merge middlewares (grup + per-route, hindari duplikasi dengan array_unique jika perlu)
//             $mergedMiddlewares = array_merge($groupMiddlewares, $route['middleware']);

//             // Tambah route yang dimodifikasi ke array utama
//             self::$routes[] = [
//                 'method' => $route['method'],
//                 'path' => $newCompiledPath,
